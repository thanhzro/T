...
void compile_func(VM *vm, const char *name, const char **params, int nparams,
    ...
    while(bi<body_count){
        ...
        if(strncmp(_bl,"F(",2)==0 && strchr(_bl,'{')){
        ...
        } else if(strncmp(_bl,"loop",4)==0 && strchr(_bl,'{')){
            /* loop { body } */
            bi++;
            const char **lb=calloc(128,sizeof(char*)); int lc=0;
            while(bi<body_count){
                const char*_bt=body[bi]; while(*_bt==' '||*_bt=='	')_bt++;
                if(_bt[0]=='}') break;
                if(body[bi][0]!=0) lb[lc++]=body[bi];
                bi++;
            }
            if(bi<body_count) bi++; /* skip } */
            ...
            for(int li=0;li<lc;li++){
                const char *ll=lb[li];
                const char *_ll=ll; while(*_ll==' '||*_ll=='	')_ll++;
                if(strncmp(_ll,"Gate ",5)==0){
                    /* Gate cond >> done = exit condition */
                    char *lp3=strchr(_ll,'('),*rp3=strchr(_ll,')');
                    if(lp3&&rp3){
                        char subj[64]={0}; strncpy(subj,_ll+5,lp3-_ll-5);
                        char *sv=subj; while(*sv==' ')sv++;
                        int _sl=strlen(sv); while(_sl>0&&sv[_sl-1]==' ')sv[--_sl]=0;
                        char inside[64]={0}; strncpy(inside,lp3+1,rp3-lp3-1);
                        char op_str[8]={0},val_str[64]={0};
                        sscanf(inside,"%s %s",op_str,val_str);
                        int isv=chunk_add_str(&f->chunk,sv);
                        chunk_write(&f->chunk,OP_LOAD); chunk_write(&f->chunk,isv);
                        /* val_str: number or variable? */
                        char *_end; double dval=strtod(val_str,&_end);
                        if(_end==val_str||*_end!=0){
                            int iv3=chunk_add_str(&f->chunk,val_str);
                            chunk_write(&f->chunk,OP_LOAD); chunk_write(&f->chunk,iv3);
                        } else {
                            int iv2=chunk_add_num(&f->chunk,dval);
                        ...
                        else if(strcmp(op_str,"<=")==0) chunk_write(&f->chunk,OP_LE);
                        /* if condition true (!=0), jump forward past loop */
                        chunk_write(&f->chunk,OP_JUMP_IF_0); chunk_write(&f->chunk,2); /* skip exit jump */
                        /* exit jump - record patch position */
                        patch_addrs[patch_count++]=f->chunk.count;
                        chunk_write(&f->chunk,OP_JUMP); chunk_write(&f->chunk,0); /* placeholder */
                    }
                } else {
                    compile_line(&f->chunk, ll);
                }
            }
            /* Unconditional backward jump to loop_start */
            int back_off = loop_start - (f->chunk.count + 2);
            chunk_write(&f->chunk,OP_JUMP); chunk_write(&f->chunk,(uint8_t)(int8_t)back_off);
            /* Patch exit jumps using recorded positions */
            int exit_pos=f->chunk.count;
            for(int pi=0;pi<patch_count;pi++){
                int pp=patch_addrs[pi];
                f->chunk.code[pp+1]=(uint8_t)(int8_t)(exit_pos-(pp+2));
            }
            free(lb);
        } else {
