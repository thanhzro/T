...
loop {
    ...
    sgd_update(weights=row0, grad=rg0, lr=lr) ~> row0
    get(arr=gg, idx=1) ~> gs1
    vec_scale(a=xa, s=gs1) ~> rg1
    sgd_update(weights=row1, grad=rg1, lr=lr) ~> row1
    get(arr=gg, idx=2) ~> gs2
    vec_scale(a=xa, s=gs2) ~> rg2
    sgd_update(weights=row2, grad=rg2, lr=lr) ~> row2
    get(arr=gg, idx=3) ~> gs3
    vec_scale(a=xa, s=gs3) ~> rg3
    sgd_update(weights=row3, grad=rg3, lr=lr) ~> row3
    linear_rows(r0=row0, r1=row1, r2=row2, r3=row3, bv=bv, x=xb) ~> pp
    ...
    sgd_update(weights=row3, grad=rg3, lr=lr) ~> row3
    linear_rows(r0=row0, r1=row1, r2=row2, r3=row3, bv=bv, x=xc) ~> pp
    ...
    sgd_update(weights=row3, grad=rg3, lr=lr) ~> row3
    linear_rows(r0=row0, r1=row1, r2=row2, r3=row3, bv=bv, x=xd) ~> pp
